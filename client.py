...
WHITE = "#F7EDDB"
FONT = ("Helvetica", 17)
BUTTON_FONT = ("new roman", 15)
SMALL_FONT = ("new roman", 13)
...
top_frame = tk.Frame(root, width=600, height=100, bg=SNOW)
top_frame.grid(row=0, column=0, sticky=tk.NSEW)

middle_frame = tk.Frame(root, width=600, height=400, bg=MEDIUM_GREY)
middle_frame.grid(row=1, column=0, sticky=tk.NSEW)

bottom_frame = tk.Frame(root, width=600, height=100, bg=SNOW)
...
message_box = scrolledtext.ScrolledText(middle_frame, font=SMALL_FONT,
                                        bg=MEDIUM_GREY, fg=WHITE, width=67,
                                        height=26.5)
message_box.config(state=tk.DISABLED)
    ...
    while 1:
        ...
        if message != '':
            ...
            messagebox.showerror("Error",
                                 "Message recevied from client is empty")
